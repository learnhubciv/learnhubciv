...
async function checkUserLicense(userId) {
    ...
    try {
        ...
        // Chercher toutes les licences pour 'tropicana'
        const q = query(
            userPurchasesRef,
        ...
        if (querySnapshot.empty) {
            ...
            showLicenseMessage("Licence non trouvée", "Vous n'avez pas acheté le prédicteur Tropicana", true);
            window.isLicenseValid = false;
            disableBotFeatures();
            return;
        }
    ...
    if (userInfoElement) {
        userInfoElement.innerHTML = `
            <div class="flex items-center">
                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-blue-400 to-cyan-400 flex items-center justify-center text-white font-bold mr-2 border border-white">
                    ${user.email ? user.email.charAt(0).toUpperCase() : 'U'}
                </div>
                <div class="text-left">
                    <p class="text-sm font-medium text-white">${user.email || 'Utilisateur'}</p>
                    <p class="text-xs text-blue-100">Prédicteur Tropicana Pro</p>
                </div>
            </div>
        `;
    }
