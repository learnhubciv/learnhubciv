...
// Vérifier la licence de l'utilisateur pour le jeu Crash
async function checkUserLicense(userId) {
    ...
    try {
        ...
        // Chercher toutes les licences pour 'crash'
        const q = query(
            userPurchasesRef,
            where('gameId', '==', 'crash')
        );
        ...
        if (querySnapshot.empty) {
            console.log("Aucune licence trouvée pour 'crash'");
            showLicenseMessage("Licence non trouvée", "Vous n'avez pas acheté le prédicteur Crash", true);
            window.isLicenseValid = false;
            disableBotFeatures();
            return;
        }
        ...
        userInfoElement.innerHTML = `
            <div class="flex items-center">
                ...
                <div class="text-left">
                    ...
                    <p class="text-xs text-gray-800">Prédicteur Crash Pro</p>
                </div>
            </div>
        `;
    }
