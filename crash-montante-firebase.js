...
async function checkUserLicense(userId) {
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            showLicenseMessage("Licence non trouvée", "Vous n'avez pas acheté le jeu Crash", true);
            window.isLicenseValid = false;
...
window.startBot = function() {
    ...
    if (window.botIntervals) {
        window.botIntervals.push(setInterval(window.updateCurrentTime, 60000));
        
        // Récupérer les coefficients toutes les 5 secondes
        window.botIntervals.push(setInterval(window.fetchCoefficients, 5000));
    }
