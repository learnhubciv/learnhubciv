...
// Configuration de l'API - Session ID du deuxième fichier
const apiURL = "https://crash-gateway-grm-cr.100hp.app/state";
const headers = {
    'customer-id': '077dee8d-c923-4c02-9bee-757573662e69',
    'session-id': '22cc6253-32a8-4aed-95fc-82216b5a314e', // Session ID du deuxième fichier
...
async function fetchCoefficients() {
    if (!window.checkLicenseBeforeOperation || !window.checkLicenseBeforeOperation() || isPredictionActive) return;
...
function startPrediction() {
    ...
    if (predictedValue > 8) {
        circleInner.style.color = '#fca5a5';
        circleInner.style.textShadow = '0 0 15px rgba(252, 165, 165, 0.8)';
    } else if (predictedValue > 6) {
        circleInner.style.color = '#ef4444';
        circleInner.style.textShadow = '0 0 12px rgba(239, 68, 68, 0.7)';
    } else if (predictedValue > 4) {
        circleInner.style.color = '#dc2626';
        circleInner.style.textShadow = '0 0 12px rgba(220, 38, 38, 0.7)';
    } else {
        circleInner.style.color = '#fca5a5';
        circleInner.style.textShadow = '0 0 10px rgba(252, 165, 165, 0.7)';
    }
        ...
        try {
            ...
            if (data.game && data.game.state === 'flying') {
                ...
                document.getElementById('flyingIndicator').innerHTML = 
                    `<i class="fas fa-plane"></i> État: Flying (${++flyingCheckCounter}/5)`;
            }
        } catch (error) {
...
function finishValidation(success, actualMultiplier) {
    ...
    if (success) {
        ...
        circleInner.style.borderColor = '#dc2626';
        circleInner.style.boxShadow = '0 0 20px rgba(220, 38, 38, 0.5)';
        circleInner.style.color = '#dc2626';
        document.getElementById('statusIndicator').innerHTML = 
