        ...
        Swal.fire({
            ...
            html: `
                <div class="text-left text-sm">
                    ...
                    <div class="mt-2 p-2 bg-gray-800 rounded-lg border border-yellow-500">
...
function updateCoefficientsDisplay() {
    ...
    if (autoCoefficients.length === 0) {
        displayElement.innerHTML = '<p class="text-center text-gray-400 text-sm">Les coefficients apparaîtront ici</p>';
        return;
    }
...
function updatePredictionDisplay(prediction) {
    ...
    document.getElementById('result').innerHTML = `
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="text-center p-3 bg-gray-800 rounded-lg border border-gray-700">
                <span class="text-xs text-gray-400 block mb-1">Prédiction Brute</span>
                <p class="text-xl font-bold text-yellow-400">${prediction.rawOdds}</p>
            </div>
            
            <div class="text-center p-3 bg-gray-800 rounded-lg border border-gray-700">
                <span class="text-xs text-gray-400 block mb-1">Prédiction Finale</span>
                <p class="text-2xl font-bold text-yellow-400">${prediction.finalOdds}</p>
            </div>
            
            <div class="text-center p-3 bg-gray-800 rounded-lg border border-green-900">
                <span class="text-xs text-gray-400 block mb-1">Probabilité de Réussite</span>
        ...
        <div class="mt-4">
            <div class="flex justify-between mb-1">
                <span class="text-sm font-medium text-gray-300">Score de prédiction</span>
...
function updatePredictionHistory() {
    ...
    if (predictionHistory.length === 0) {
        historyContainer.innerHTML = `
            <div class="text-center text-gray-400 p-4">
                <i class="fas fa-chart-line text-2xl text-gray-500 mb-2"></i>
    ...
    historyContainer.innerHTML = predictionHistory.map((prediction) => {
        ...
        return `
        <div class="history-item bg-gray-800 border border-gray-700 p-3 rounded-lg">
            <div class="flex justify-between items-start">
                <div class="flex-1">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="font-semibold text-gray-300 text-sm">${prediction.date}</p>
            ...
            Swal.fire({
                ...
                html: `
                    <div class="mb-4">
                        ...
                        <label class="block text-gray-300 mb-2">À quel round la prédiction a-t-elle été validée? (1-3)</label>
                        <select id="roundSelect" class="w-full p-2 border rounded bg-gray-800 text-white">
                            ${[1,2,3].map(num => 
        ...
        html: `
            <div class="grid grid-cols-1 gap-3 text-sm">
                <div class="text-left">
                    ...
                    <select id="analysisMode" class="w-full p-2 border border-gray-600 rounded-lg bg-gray-800 text-white">
                ...
                <div class="text-left">
                    ...
                    <div class="space-y-1">
                        <label class="flex items-center p-1.5 bg-gray-800 rounded-lg text-gray-300">
                ...
                <div class="text-left">
                    <label class="block text-gray-300 mb-1">Seuil de volatilité</label>
                    <input type="range" id="volatilityThreshold" min="0.05" max="0.5" step="0.01" 
                        value="${currentSettings.volatilityThreshold}" class="w-full">
                    <div class="text-center text-xs text-gray-400 mt-1" id="volatilityValue">
...
function analyzeHistoricalData() {
    ...
    if (!window.isLicenseValid) {
        document.getElementById('intelligentAnalysis').innerHTML = `
            <div class="text-center text-gray-400 p-3">
                <i class="fas fa-lock text-2xl text-gray-500 mb-2"></i>
    ...
    if (successfulPredictions.length === 0) {
        document.getElementById('intelligentAnalysis').innerHTML = `
            <div class="text-center text-gray-400 p-3 text-xs">
                <i class="fas fa-database text-2xl text-gray-500 mb-2"></i>
    ...
    document.getElementById('intelligentAnalysis').innerHTML = `
        <div class="space-y-3">
            ...
            <div class="p-2 bg-gray-800 rounded-lg border border-green-900">
                ...
                <div class="mt-1">
                    <p class="text-xs text-gray-400 mb-1">Répartition des succès (après marge):</p>
                    <div class="space-y-1">
                        <div class="flex items-center">
                            <span class="text-xs w-12 text-gray-400">1.0-1.5:</span>
                            <div class="flex-1 bg-gray-700 rounded-full h-1.5">
                                <div class="bg-green-500 h-1.5 rounded-full" style="width: ${(oddsRanges['1.0-1.5'] / successfulPredictions.length) * 100}%"></div>
                            </div>
                            <span class="text-xs w-6 text-right text-gray-300">${oddsRanges['1.0-1.5']}</span>
                        </div>
                        <div class="flex items-center">
                            <span class="text-xs w-12 text-gray-400">1.5-2.0:</span>
                            <div class="flex-1 bg-gray-700 rounded-full h-1.5">
                                <div class="bg-green-500 h-1.5 rounded-full" style="width: ${(oddsRanges['1.5-2.0'] / successfulPredictions.length) * 100}%"></div>
                            </div>
                            <span class="text-xs w-6 text-right text-gray-300">${oddsRanges['1.5-2.0']}</span>
                        </div>
                        <div class="flex items-center">
                            <span class="text-xs w-12 text-gray-400">2.0-2.5:</span>
                            <div class="flex-1 bg-gray-700 rounded-full h-1.5">
                                <div class="bg-green-500 h-1.5 rounded-full" style="width: ${(oddsRanges['2.0-2.5'] / successfulPredictions.length) * 100}%"></div>
                            </div>
                            <span class="text-xs w-6 text-right text-gray-300">${oddsRanges['2.0-2.5']}</span>
                        </div>
                        <div class="flex items-center">
                            <span class="text-xs w-12 text-gray-400">2.5+:</span>
                            <div class="flex-1 bg-gray-700 rounded-full h-1.5">
                                <div class="bg-green-500 h-1.5 rounded-full" style="width: ${(oddsRanges['2.5+'] / successfulPredictions.length) * 100}%"></div>
                            </div>
                            <span class="text-xs w-6 text-right text-gray-300">${oddsRanges['2.5+']}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-2 bg-gray-800 rounded-lg border border-yellow-900">
                <div class="font-bold text-yellow-400 mb-1 text-sm">
