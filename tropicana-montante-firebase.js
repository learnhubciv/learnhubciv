...
// Configuration Firebase (MÊME QUE DANS tropicana-predictor.html)
...
// Vérifier la licence de l'utilisateur pour le jeu Tropicana
async function checkUserLicense(userId) {
    ...
    try {
        ...
        // REQUÊTE POUR 'tropicana' comme dans le premier fichier
        const q = query(
            userPurchasesRef,
            where('gameId', '==', 'tropicana')
        );
        ...
        if (querySnapshot.empty) {
            console.log("Aucune licence trouvée pour 'tropicana'");
            showLicenseMessage("Licence non trouvée", "Vous n'avez pas acheté le jeu Tropicana", true);
...
function showLicenseMessage(title, message, isError = false) {
    ...
    messageDiv.innerHTML = `
        ...
        <br><button id="goToStore" style="margin-top: 10px; padding: 8px 16px; background: white; color: #38bdf8; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">Acheter une licence</button>
    `;
        ...
        userInfoElement.innerHTML = `
            <div class="flex items-center">
                ...
                <div class="text-left">
                    ...
                    <p class="user-subtitle">Prédicteur Tropicana Pro</p>
                </div>
            </div>
        `;
    }
...
function startBot() {
    ...
    botIntervals.push(setInterval(updateCurrentTime, 60000));
    
    // Récupérer les coefficients toutes les 5 secondes
    botIntervals.push(setInterval(fetchCoefficients, 5000));
...
window.userLicense = userLicense;
window.isLicenseValid = isLicenseValid;
window.botIntervals = botIntervals;
