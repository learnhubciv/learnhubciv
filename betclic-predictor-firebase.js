...
// Vérifier la licence de l'utilisateur pour le jeu betclic
async function checkUserLicense(userId) {
    ...
    try {
        ...
        // Chercher toutes les licences pour 'betclic'
        const q = query(
            userPurchasesRef,
            where('gameId', '==', 'betclic')
        );
        ...
        if (querySnapshot.empty) {
            console.log("Aucune licence trouvée pour 'betclic'");
            showLicenseMessage("Licence non trouvée", "Vous n'avez pas acheté le prédicteur Betclic", true);
            window.isLicenseValid = false;
            disableBotFeatures();
            return;
        }
    ...
    if (resultElement) {
        resultElement.innerHTML = `
            <div class="text-center p-4">
                <i class="fas fa-key text-3xl text-red-300 mb-3"></i>
...
function enableBotFeatures() {
    ...
    if (licenseIndicator) {
        ...
        licenseIndicator.innerHTML = `
            <span class="bg-white rounded-full w-2 h-2 inline-block mr-1"></span>
    ...
    if (betInput) {
        ...
        betInput.placeholder = "Montant à miser";
    }
}
    ...
    if (userInfoElement) {
        userInfoElement.innerHTML = `
            <div class="flex items-center">
                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-red-600 to-red-800 flex items-center justify-center text-white font-bold mr-2 border border-white">
                    ${user.email ? user.email.charAt(0).toUpperCase() : 'U'}
                </div>
                <div class="text-left">
                    <p class="text-sm font-medium text-white">${user.email || 'Utilisateur'}</p>
                    <p class="text-xs text-red-100">Prédicteur Betclic Pro</p>
                </div>
            </div>
        `;
    }
