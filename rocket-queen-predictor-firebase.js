...
async function checkUserLicense(userId) {
    ...
    try {
        ...
        // Chercher toutes les licences pour 'rocketqueen'
        const q = query(
            userPurchasesRef,
            where('gameId', '==', 'rocket-queen')
        );
    ...
    if (userInfoElement) {
        userInfoElement.innerHTML = `
            <div class="flex items-center">
                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-purple-600 to-purple-400 flex items-center justify-center text-white font-bold mr-2 border border-purple-300">
                    ${user.email ? user.email.charAt(0).toUpperCase() : 'U'}
                </div>
                <div class="text-left">
                    <p class="text-sm font-medium text-white">${user.email || 'Utilisateur'}</p>
                    <p class="text-xs text-purple-200">Prédicteur Rocket Queen Pro</p>
                </div>
            </div>
        `;
    }
