...
// Vérifier la licence de l'utilisateur pour le jeu Rocket X
async function checkUserLicense(userId) {
    ...
    try {
        ...
        // Chercher toutes les licences pour 'rocket-x'
        const q = query(
            userPurchasesRef,
            where('gameId', '==', 'rocket-x')
        );
        ...
        if (querySnapshot.empty) {
            console.log("Aucune licence trouvée pour 'rocket-x'");
            showLicenseMessage("Licence non trouvée", "Vous n'avez pas acheté le prédicteur Rocket X", true);
            window.isLicenseValid = false;
            disableBotFeatures();
            return;
        }
...
function disableBotFeatures() {
    ...
    if (licenseIndicator) {
        licenseIndicator.innerHTML = `
            <span class="bg-red-500 rounded-full w-2 h-2 inline-block mr-1"></span>
            <span class="text-black font-medium">Licence expirée</span>
        `;
    }
    ...
    if (resultElement) {
        resultElement.innerHTML = `
            <div class="text-center p-4">
                <i class="fas fa-key text-3xl text-yellow-400 mb-3"></i>
...
function enableBotFeatures() {
    ...
    if (licenseIndicator) {
        ...
        licenseIndicator.innerHTML = `
            ...
            <span class="text-black font-medium">Licence: ${remainingHours}h ${remainingMinutes}m</span>
        `;
    }
    ...
    if (userInfoElement) {
        userInfoElement.innerHTML = `
            <div class="flex items-center">
                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-yellow-400 to-yellow-600 flex items-center justify-center text-black font-bold mr-2 border border-white">
                    ${user.email ? user.email.charAt(0).toUpperCase() : 'U'}
                </div>
                <div class="text-left">
                    <p class="text-sm font-medium text-black">${user.email || 'Utilisateur'}</p>
                    <p class="text-xs text-gray-800">Prédicteur Rocket X Pro</p>
                </div>
            </div>
        `;
    }
