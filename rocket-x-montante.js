...
// Configuration de l'API - Utilisation de la session_id du premier fichier Rocket X
const apiURL = "https://crash-gateway-grm-cr.100hp.app/state";
const headers = {
    'customer-id': '077dee8d-c923-4c02-9bee-757573662e69',
    'session-id': 'efe4a167-55b2-4bd3-a137-1f3faf6e4fe0', // Session ID du premier fichier Rocket X
...
function startPrediction() {
    ...
    if (predictedValue > 8) {
        circleInner.style.color = '#FDE68A';
        circleInner.style.textShadow = '0 0 15px rgba(253, 230, 138, 0.8)';
    } else if (predictedValue > 6) {
        circleInner.style.color = '#FBBF24';
        circleInner.style.textShadow = '0 0 12px rgba(251, 191, 36, 0.7)';
        ...
        circleInner.style.textShadow = '0 0 10px rgba(146, 64, 14, 0.7)';
    }
...
function updateCountdown(secondsLeft) {
    ...
    if (secondsLeft <= 10 && secondsLeft > 0) {
        document.getElementById('countdown').style.color = '#F59E0B';
...
// Démarrer la vérification de l'état Flying (Vol)
        ...
        try {
            ...
            if (data.game && data.game.state === 'flying') {
                ...
                document.getElementById('flyingIndicator').innerHTML = 
                    `<i class="fas fa-rocket"></i> État: Vol en cours (${++flyingCheckCounter}/5)`;
            }
        } catch (error) {
...
async function checkPredictionResult() {
    ...
    try {
        ...
        if (gameState === 'flying') {
            ...
            document.getElementById('statusIndicator').innerHTML = 
                `<i class="fas fa-clock"></i> ATTENTE DU PROCHAIN JEU`;
...
function finishValidation(success, actualMultiplier) {
    ...
    if (success) {
        ...
        circleInner.style.borderColor = '#F59E0B';
        circleInner.style.boxShadow = '0 0 20px rgba(245, 158, 11, 0.5)';
        circleInner.style.color = '#F59E0B';
        document.getElementById('statusIndicator').innerHTML = 
